...
exo <- read.csv("KorEXO Measurement File Export - 031824 030139.csv") |>
  ...
  mutate(color = factor(color, levels = levels(color), labels = sample(colours(), length(levels(color)), replace = FALSE))) |> 
  mutate(
...
icons_list <- lapply(unique(exo$color), function(col) {
  awesomeIcons(
    icon = 'ios-close',  # Example icon
    iconColor = 'white',  # Icon color
    library = 'ion',  # Icon library
    markerColor = col  # Here you use each color from your df
  )
})
names(icons_list) <- unique(exo$color)
